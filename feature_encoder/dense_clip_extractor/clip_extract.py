...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="Paths to images for which to extract CLIP features",
    )
    parser.add_argument(
        "--device",
        type=str,
        default="cuda" if torch.cuda.is_available() else "cpu",
        help="Device to run CLIP on (default: %(default)s)",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default="clip_features.pt",
        help="Path to save extracted CLIP features (default: %(default)s)",
    )
    args = parser.parse_args()

    embeddings = extract_clip_features(args.image_paths, torch.device(args.device))
