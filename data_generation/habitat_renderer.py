...
def init_habitat(config) :
    ...
    for sensor_uuid, sensor_params in sensors.items():
        if config[sensor_uuid]:
            sensor_spec = hs.CameraSensorSpec()
...
def main():
    ...
    for scene in base_config["scenes_list"]:
        ...
        if config["lighting"]:
            ...
            config["scene_file"] = os.path.join(config["root_path"], scene, "habitat/mesh_semantic.ply")

        config["instance2class_mapping"] = os.path.join(config["root_path"], scene, config["info_semantic_path"])
        config["pose_file"] = os.path.join(config["save_root_path"], scene, "traj.txt")
        config["save_path"] = os.path.join(config["save_root_path"], scene)
...
def main_single(config):
    ...
    for i in tqdm(range(total_render_num), desc="Rendering Process", unit="frame"):
